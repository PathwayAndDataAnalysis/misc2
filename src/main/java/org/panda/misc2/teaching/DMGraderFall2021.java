 ...
 */
public class DMGraderFall2021
{
	public static void main(String[] args) throws IOException
	{
		String dir = "/home/ozgunbabur/Documents/Teaching/DiscreteMath/Fall2021/";
		Map<String, Double> grades = getOverallGrades(dir + "CS_220_Fall_2021_grades.csv",
			dir + "gc_B3110-4314_fullgc_2021-12-23-14-58-19.tsv",
			dir + "UMBCS220BaburFall2021_assignment_report_2021-12-23_1126.csv",
			dir + "extra-credit.txt");
...
//			.sorted()
	...
	private static Map<String, Double> getOverallGrades(String gradescopeFile, String blackboardFile, String zyBookFile, String extraFile) throws IOException
	{
		...
		Map<String, Double> quizGrades = readQuizGradeAvg(blackboardFile);
		Map<String, Double> extraGrades = readExtraCredit(extraFile);

		List<String> names = mfGrades.keySet().stream().sorted().collect(Collectors.toList());
		...
		for (String name : names)
		{
			...
			double overall = (0.25 * midterm) + (0.4 * finalExam) + (0.05 * quiz) + (0.15 * book) + (0.15 * hw);
			gradesMap.put(name, overall);
		}

		shiftCurveBy(gradesMap, 21);

		extraGrades.keySet().stream().forEach(name -> gradesMap.put(name, gradesMap.get(name) + extraGrades.get(name)));
